...
export default function Home() {
  ...
  const [selectedTopics, setSelectedTopics] = useState<string[]>(["DSA", "System Design"]);
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (res.output?.success) {
        ...
        // Build studentProfile with the LLM response data + user inputs
        const studentProfile = {
          ...
          academicYear: profile.academicYear,
          targetCompany: profile.targetCompany,
          // LLM analysis results (snake_case from agent)
          resume_score: data.resume_score || 0,
          project_depth_score: data.project_depth_score || 0,
          technical_maturity_score: data.technical_maturity_score || 0,
          missing_core_areas: data.missing_core_areas || [],
          inferred_strengths: data.inferred_strengths || [],
        ...
        setAppState((prevState: any) => ({
          ...
          readinessScore: data.resume_score ? data.resume_score * 10 : 50,
          skillGapData: { priorityGaps: data.missing_core_areas || [] }
        }));
        
        toast({
          title: "Analysis Complete! 🎉",
          description: `Resume Score: ${data.resume_score}/10 | Found ${data.missing_core_areas?.length || 0} areas to improve`
        });
  ...
  const handleStartInterview = async () => {
    ...
    toast({
      ...
      description: `Topics: ${selectedTopics.join(', ')}`
    });
  ...
  const handleGeneratePlan = async () => {
    ...
    if (res.output?.data) {
      setAppState((prev: any) => ({ ...prev, plan: res.output.data }));
      toast({
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-7xl mx-auto">
              ...
              {view === "DASHBOARD" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-8">
                    ...
                    >
                      {loading ? "Starting..." : "🎯 Start Adaptive Interview (Medium)"}
                    </Button>
              ...
              {view === "INTERVIEW" && (
                <InterviewInterface
                  ...
                  loading={loading}
                  onDifficultyChange={(diff) => console.log("Diff changed", diff)}
